...
export class GitHubStore {
  constructor(fetcher=fetch){this.fetcher=fetcher;this.token='';}
  ...
  async request(url,options={}){
    ...
    if(!response.ok){if(response.status===401)throw new Error('接続キーが無効か、有効期限が切れています。再接続してください。');if(response.status===409)throw new Error('保存中に別の変更が入りました。もう一度保存してください。');if(response.status===403||response.status===429)throw new Error('GitHubの権限不足、またはアクセス制限です。接続キーの権限を確認するか、時間を置いて再試行してください。');throw new Error(`GitHubへの保存・取得に失敗しました（${response.status}）。`);}
  ...
  async connect(token){
    ...
    try{const repo=await this.request('https://api.github.com/repos/4k29/want');if(!repo.permissions?.push)throw new Error('wantへの書き込み権限がある接続キーを入力してください。');await this.read();}catch(error){this.token='';throw error;}
  }
